class Splash {
  constructor() {
    ...
    this.info = createDiv(
      "This is a program you can use to write midi data and play it using oscillators. <p> Click the + button to add a midi or oscillator panel, you can connect them to each other by clicking and dragging over the nodes.<p>Yellow nodes = MIDI input/output <p> Blue nodes = sound input/output<p> The blue dot all the way to the right is the master output.<p> You can connect the sound output of an oscillator to the sound input of another oscillator to do FM/AM modulation. Double click the circle in a panel to open up the editor for that panel.<p> Midi panels can have different beat lengths, as well as different tempos.<p> The speed multiplier can be used to make polyrhythms. <p>Try experimenting with it!<p> <a href=https://https://editor.p5js.org/louischavasse1/sketches/hbKFWVKZpd>view code</a>"
    );
